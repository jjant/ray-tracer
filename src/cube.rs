...
impl Cube {
    pub fn local_intersect(local_ray: Ray) -> Vec<f64> {
        let (xt_min, xt_max) = check_axis(local_ray.origin.x, local_ray.direction.x);
        let (yt_min, yt_max) = check_axis(local_ray.origin.y, local_ray.direction.y);
        let (zt_min, zt_max) = check_axis(local_ray.origin.z, local_ray.direction.z);
